...
//		projectClientV2 = OSFactory.builderV2().endpoint("http://192.168.100.11:5000/v2.0").credentials("admin", "631a1ae0e8964e3c")
...
//		config.setAodhServiceUrl("http://192.168.101.151:8042/v2/alarms");
		...
		CloudManipulator cloud = CloudManipulatorFactory.createCloudManipulator(config, config.getAdminProjectId());
...
//		System.out.println(absolute.getTotalGigabytesUsed());
	...
	private static void showFloatingIpPorts() {
		List<? extends Port> ports = projectClientV3
				.networking()
				.port()
				.list(PortListOptions.create().networkId("01f12475-3687-4214-9d6d-8f2d46447eb1")
	...
	private static String escapeNonAscii(String inputString) {
		/*
		 * The characters with values that are outside of the 16-bit range, and within the range from 0x10000 to
		 * 0x10FFFF, are called supplementary characters and are defined as a pair of char values.
		 * http://stackoverflow.com/questions/5733931/java-string-unicode-value
		 */
	...
	private static void showSamples() throws ParseException {
		...
		SampleCriteria criteria = new SampleCriteria().resource(resourceId).timestamp(SampleCriteria.Oper.GT,
				timestamp);
		List<? extends MeterSample> meterSamples = projectClientV3.telemetry().meters().samples(meterName, criteria);
