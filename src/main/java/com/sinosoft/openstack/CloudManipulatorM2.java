...
public class CloudManipulatorM2 extends CloudManipulatorM {
  ...
  public CloudManipulatorM2(CloudConfig appConfig, String projectId) {
    ...
    projectClientM2 = OSFactory.builderV2().endpoint(OS_AUTH_URL)
        .credentials(OS_USERNAME, OS_PASSWORD).tenantId(projectId).authenticate();
  ...
  public String createProject(String projectName, String projectDescription, int instanceQuota,
    ...
    try {
      OSClientV2 client = OSFactory.builderV2().endpoint(OS_AUTH_URL)
          .credentials(OS_USERNAME, OS_PASSWORD).tenantName(OS_TENANT_NAME)
          .perspective(Facing.ADMIN).authenticate();

      // create tenant
      Tenant tenant = client.identity().tenants().create(
          Builders.identityV2().tenant().name(projectName + "_" + UUID.randomUUID().toString())
              .description(projectDescription).build());
      tenantId = tenant.getId();

      // set quota
      client.compute().quotaSets().updateForTenant(tenantId, Builders.quotaSet()
      ...
      Subnet subnet = client.networking().subnet()
          .create(Builders.subnet().name("private_subnet").networkId(network.getId())
              .tenantId(tenantId).ipVersion(IPVersionType.V4).cidr("192.168.32.0/24")
              .gateway("192.168.32.1").enableDHCP(true).build());
      Router router = client.networking().router().create(Builders.router().name("router")
          .adminStateUp(true).externalGateway(PUBLIC_NETWORK_ID).tenantId(tenantId).build());
      ...
      OSClientV2 tenantClient = OSFactory.builderV2().endpoint(OS_AUTH_URL)
          .credentials(OS_USERNAME, OS_PASSWORD).tenantId(tenantId).authenticate();
  ...
  public QuotaSet updateComputeServiceQuota(int instanceQuota, int cpuQuota, int memoryQuota) {
    try {
      ...
      QuotaSet quota = client.compute().quotaSets().updateForTenant(projectId, Builders.quotaSet()
          .cores(cpuQuota).instances(instanceQuota).ram(memoryQuota * 1024).build());
  ...
  public Image updateImage(String imageId, String imageName, boolean publicity) {
    ...
    Image newImage = projectClientM2.images()
        .update(image.toBuilder().name(imageName).isPublic(publicity).build());
    return newImage;
  }

  /*
   * openstack4j api not work for M, use it for K only.
   */
  // @Override
  ...
  // // alarm.toBuilder().isEnabled(enabled).thresholeRule((CeilometerThresholdRule)
  // modifiedRule).build());
  //
  // return true;
  // }
